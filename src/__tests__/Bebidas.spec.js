...
test('26 - Carregue as 12 primeiras receitas de comidas ou bebidas, uma em cada card', async () => {
    ...
    recipeCard0 =  await utils.findByTestId('0-recipe-card');
    imgCard0 = await utils.findByTestId('0-card-img');
    nameCard0 =  await utils.findByTestId('0-card-name');
    recipeCard12 =  await utils.findByTestId('11-recipe-card');
    imgCard12 = await utils.findByTestId('11-card-img');
    nameCard12 =  await utils.findByTestId('11-card-name');
    expect(recipeCard0).toBeInTheDocument();
    expect(imgCard0).toBeInTheDocument();
    expect(nameCard0).toBeInTheDocument();
    expect(recipeCard12).toBeInTheDocument();
