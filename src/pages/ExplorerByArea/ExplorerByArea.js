...
useEffect(()=>{
  const getCategories = ()=>{
    if(valor){
      fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${valor}`).then(data=>data.json()).then(({meals})=>setFilterArea(meals))
...
return(
  ...
  <>
    <Header title={ title } />
    <select data-testid="explore-by-area-dropdown" onChange={(e)=>setValor(e.target.value)}>
      {categories && categories.map(({strArea},index)=>{
        return(
        <option data-testid={`${strArea}-option`} key={index}>{strArea}</option>
        )
      })}
    </select>
{filterArea && filterArea.map(({idMeal,strMeal,strMealThumb},index)=>{
  if(index < 12){
