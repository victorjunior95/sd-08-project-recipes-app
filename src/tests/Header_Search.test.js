...
describe(`Implemente os elementos da barra de busca
  ...
  it('Se os elementos do search estão presentes', () => {
    ...
    expect(screen.queryByTestId(INGREDIENT_RADIO)).toBeInTheDocument();
    expect(screen.queryByTestId(NAME_RADIO)).toBeInTheDocument();
    expect(screen.queryByTestId(FIRST_LETTER_RADIO)).toBeInTheDocument();
    expect(screen.queryByTestId(EXEC_BTN)).toBeInTheDocument();
  });
});

describe.only(`Posicione a barra logo abaixo do header e implemente
  3 radio buttons: Ingrediente, Nome e Primeira letra`, () => {
  it(`Se o radio selecionado for Ingrediente, a busca na
    API é feita corretamente pelo ingrediente`, () => {
    ...
    cy.get('[data-testid=INGREDIENT_RADIO]').click();
